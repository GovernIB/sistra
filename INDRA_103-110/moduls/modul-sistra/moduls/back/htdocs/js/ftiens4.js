...
function irANodo(linkID){	
	docObj = findObj(linkID);		
	if (docObj == null){
		return;
	}
	if ( docObj.children == undefined){
		loadSynchPage(linkID);
	}else{
		openFolderInTree(linkID);
	}
}
...
{ 
  ...
  { 
    ...
    docW = docW + this.desc + "</a> " + this.pospendHTML
  } 
  else 
    docW = docW + this.desc + " " + this.pospendHTML
  docW = docW + "</td>"
  ...
  {
	  if (supportsDeferral) {
		  doc.write("<div id=domRoot style=\"width: 80%;padding-left: 4em;\"></div>") //transition between regular flow HTML, and node-insert DOM DHTML
