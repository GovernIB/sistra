...
// TODO FLUJO TRAMITACION PARA PAGOS Y REGISTRO
...
public class TramiteProcessorEJB implements SessionBean {
	...
	public void ejbActivate() throws EJBException, RemoteException {
		...
		log.info("ejbActivate  - [Id Persistencia: " + id + "]");
	...
	public void ejbPassivate() throws EJBException, RemoteException {
		...
		log.info("ejbPassivate - [Id Persistencia: " + id + "]");
	}
	 ...
	 * @ejb.permission role-name="${role.user}"
	 */
	public void ejbCreate(String tramite, int version, char nivelAutenticacion, Locale idioma, Map parametrosInicio) throws CreateException {		
		log.debug("ejbCreate");
		try{
			// Obtenemos datos usuario
			obtenerDatosSesion(nivelAutenticacion,idioma);
	...
	public RespuestaFront informacionInicial(){		
		try{
    		log.debug(mensajeLog("Obtiene información para mostrar el login"));     	
	...
	public RespuestaFront obtenerTramitesPersistencia(String tramite,int version){		
		try{
			log.debug(mensajeLog("Obtener lista de trámites en zona persistencia"));    
    ...
    public RespuestaFront cargarTramite(String idPersistencia) {
    	...
    	// TODO QUEDARIA VER CUANDO SE RECUPERA UN TRAMITE DE PERSISTENCIA COMO SE ALIMENTA EL VALOR DE FIRMADO DE FORMULARIOS Y ANEXOS
    	//	AHORA MISMO SE ESTABLECE AL CALCULAR ESTADO DOC INFO	
    	...
    	try{    		
    		...
    		log.debug(mensajeLog("Cargar tramite persistente " + idPersistencia + " con nivel autenticacion " + datosSesion.getNivelAutenticacion())); 
    	...
    	try{
	    	...
	    	for (int i=0;i<paso;i++){
	    		...
	    		if (!pasoAct.getCompletado().equals(PasoTramitacion.ESTADO_COMPLETADO)) {
	    			...
	    			// Si vamos a paso anexar, dejamos pasar si el paso rellenar esta en estado PENDIENTE_FLUJO
    ...
    public RespuestaFront confirmarPago(String identificador,int instancia) {  
    	...
    	try{	    		    	
    		...
    		if (confirmado){
    			...
    			if (datosPago.getTipoPago() == 'T') 
    				logAuditoria(ConstantesAuditoria.EVENTO_PAGO_TELEMATICO_CONFIRMADO,"S","",datosPago.getNumeroDUI());
    		}
    	}
    }
    ...
    /**
     * Anular pago :
     * 	- Un pago telemático se puede anular si esta iniciado pero no terminado
     *  - Un pago presencial se puede anular si esta iniciado o terminado
     *  
     *  
     * @ejb.interface-method    
     * @ejb.permission role-name="${role.user}"
    ...
    public RespuestaFront anularPago(String identificador,int instancia) {    	
    	...
    	boolean anulado = false;   // Indica si se ha anulado pago
    	boolean confirmado = true; // Indica que el pago se ha confirmado
    	...
    	try{
	    	...
	    	// Procedemos segun el estado del documento
	    	switch (docPAD.getEstado()){
	    		case DocumentoPersistentePAD.ESTADO_NORELLENADO:
	    			// Sea presencial o telemático sólo se podrá anular si ha sido iniciado
	    	    	throw new Exception("Se ha invocado a anular Pago y el pago no ha sido iniciado");	    	    	
	    		case DocumentoPersistentePAD.ESTADO_CORRECTO:
	    			// Solo se admite xa presencial
	    			if (datosPago.getEstado() == 'T'){
	    	    		throw new Exception("Se ha invocado a anular Pago y el pago es telemático y ha sido finalizado");
	    	    	}	    			
	    			break;
	    		case DocumentoPersistentePAD.ESTADO_INCORRECTO:
	    			// Si el pago esta iniciado antes comprobamos su estado
	    	    	PluginPagosIntf pluginPago = PluginFactory.getInstance().getPluginPagos();
	    	    	EstadoSesionPago estadoSesionPago = pluginPago.comprobarEstadoSesionPago(datosPago.getLocalizador());
	    	    	
	    	    	MensajeFront mens;	    				
    				switch (estadoSesionPago.getEstado()){
    					case ConstantesPago.SESIONPAGO_NO_EXISTE_SESION:
    						// No existe sesion de pago. Dejamos anular pago.
    						break;
    					case ConstantesPago.SESIONPAGO_EN_CURSO:
    						// Sesion en curso. Dejamos anular pago.
    						// -- Finalizamos sesion de pago en el plugin
    						pluginPago.finalizarSesionPago(datosPago.getLocalizador());
    						break;
    					case ConstantesPago.SESIONPAGO_PAGO_PENDIENTE_CONFIRMAR:
    						// Pago pendiente de confirmacion. No dejamos anular.
    						mens = new MensajeFront();
	    		    		mens.setTipo(MensajeFront.TIPO_WARNING);
	    		    		mens.setMensaje(traducirMensaje(MensajeFront.MENSAJE_ANULARPAGOPENDIENTECONFIRMAR));	    		    		
	    		    		mens.setMensajeExcepcion("No se puede anular el pago porque esta pendiente de confirmacion");
	    		    		return generarRespuestaFront(mens,null);   	 	
    					case ConstantesPago.SESIONPAGO_PAGO_CONFIRMADO:
    						// Pago ha sido realizado
    						// - Si es presencial dejamos anularlo. Finalizamos sesion de pago en plugin.
    						if (estadoSesionPago.getTipo() == 'P'){
    							pluginPago.finalizarSesionPago(datosPago.getLocalizador());
    						}else{    						
	    						// Si es telematico no dejamos anular.
	    						//  -- Actualizamos datos pago
	    	    		    	datosPago.setTipoPago(estadoSesionPago.getTipo());
	    	    		    	datosPago.setNumeroDUI(estadoSesionPago.getIdentificadorPago());
	    	    		    	datosPago.setJustificantePago(estadoSesionPago.getReciboB64PagoTelematico());
		    			    	...
		    			    	mens = new MensajeFront();
		    		    		mens.setTipo(MensajeFront.TIPO_WARNING);
		    		    		mens.setMensaje(traducirMensaje(MensajeFront.MENSAJE_ANULARPAGOTELEMATICOREALIZADO));	    		    		
		    		    		mens.setMensajeExcepcion("No se puede anular el pago telemático porque ya esta pagado");
		    		    		return generarRespuestaFront(mens,null);   	 	 
    						}
	    		    	default:
	    		    		throw new Exception("Estado sesion de pago desconocido: " + estadoSesionPago.getEstado());
    				}	    	    	
	    	    	break;		    	    
	    	}
	    	...
	    	// Indicamos que el pago se ha anulado
	    	anulado = true;
    		...
    		// Auditamos si anulamos pago y es telematico
    		if (anulado && datosPago.getTipoPago() == 'T')
    			logAuditoria(ConstantesAuditoria.EVENTO_PAGO_TELEMATICO_ANULADO,"S","",datosPago.getNumeroDUI());
    		// Auditamos si se confirma
    		if (confirmado){
    			logAuditoria(ConstantesAuditoria.EVENTO_PAGO,"S","Pago confirmado",Character.toString(datosPago.getTipoPago()));
    			if (datosPago.getTipoPago() == 'T') 
    				logAuditoria(ConstantesAuditoria.EVENTO_PAGO_TELEMATICO_CONFIRMADO,"S","",datosPago.getNumeroDUI());
    ...
    public void finalizarSesionPago(String identificador,int instancia) {    	
    	...
    	try{
	    	...
	    	if (docPAD.getEstado() != DocumentoPersistentePAD.ESTADO_CORRECTO){
	    		log.info("No se puede finalizar sesión de pago si el pago no se ha finalizado");
	    		return;
	    	}
    ...
    public RespuestaFront guardarFormulario(String identificador,int instancia,String datosAnteriores,String datosNuevos) {
    	try{	
	    	...
	    	if (!datosForm.getString().equals(datosAnteriores)){	    	
	    		throw new ProcessorException("Los datos almacenados para el formulario " + ls_id + " no coinciden",MensajeFront.MENSAJE_ERRORDESCONOCIDO);
	    	}	    	
	    	...
	    	datosFormularios.put(ls_id,datosFormNuevos);
	    	...
	    	// Estado anexos con script de flujo: si en el script de flujo esta el formulario los borraremos
     ...
     * @ejb.permission role-name="${role.user}"
     */
    public RespuestaFront firmarFormulario(String identificador,int instancia,FirmaIntf firma) {
    	try{
	    	...
	    	// Cacheamos firma
	    	this.firmaDocumentos.put(docPad.getRefRDS().toString(),firma);	    		    	
     ...
     * @return
     */
    public RespuestaFront anexarDocumento(String identificador,int instancia,byte[] datosDocumento,String nomFichero,String extension,String descPersonalizada,FirmaIntf firma) {
    	//TODO: Contemplar anexo de documentos desde la PAD
    	//TODO: Blindar para q no puedan anexarse docs q pertenecen a otro flujo
    	try{	    	
    		// Comprobamos que estamos en el paso de anexar
    		PasoTramitacion pasoRellenar = (PasoTramitacion) pasosTramitacion.get(pasoActual);
	    	...
	    	if (docPad == null){
	    		// Debe ser genérico
	    		if (doc.getGenerico() != 'S') throw new Exception("El anexo" + doc.getIdentificador() + " no existe en la zona de persistencia y no es genérico");
    ...
    public String obtenerIdPersistencia()
    {
		...
		log.debug("obtenerIdPersistencia: " + id);
    	return id;
    }
...
//  Borramos anexo actualizando RDS y PAD, y evaluando de nuevo el estado del paso de Anexar
    ...
    private void validarAcceso() throws Exception{    	
		...
		if (script != null && script.length > 0){
			...
			log.debug(mensajeLog("Acceso denegado por no pasar validación inicio"));	
    ...
    private void inicializarTramite() throws Exception{    	
    	try{    		    	
	    	...
	    	String idPersistencia = actualizarPAD();
	    	...
	    	for (Iterator it=formularios.iterator();it.hasNext();){
	    		...
	    		if (docPad.getRefRDS() != null){
	    			...
	    			if (!doc.getModelo().equals(docRds.getModelo())){
	    				log.debug("XML RDS - modelo \n" + docRds.getModelo());
	    				log.debug("XML GENERADO - modelo: \n" + doc.getModelo() );
	    			...
	    			if (docNivel.getVersion() != docRds.getVersion()){
	    				log.debug("XML RDS - version \n" + docRds.getVersion());
	    				log.debug("XML GENERADO - version: \n" + docNivel.getVersion() );
	    			...
	    			if (docNivel.getFirmar() == 'S'){		    			
						...
						{																
							this.firmaDocumentos.put(docRds.getReferenciaRDS().toString(),docRds.getFirmas()[0]);
						}
	    			}
	    		...
	    		for (int i=1;i<=numInst;i++){
		    		...
		    		if (docPad.getRefRDS() != null){	    				    			
		    			...
		    			if (docNivel.getFirmar() == 'S'){
							...
							if ( docRds.getFirmas() != null && docRds.getFirmas().length > 0 )
							{
								this.firmaDocumentos.put(docPad.getRefRDS().toString(),docRds.getFirmas()[0]);							
							}
		    			}
    ...
    private void calcularFlujoTramitacion(TramiteFront tramite) throws Exception{
    	...
    	if (tramite.isFlujoTramitacion()) {
			...
			if (ultimoPasoCompletado < tramite.getPasoNoRetorno() && pendienteFlujo){		
				...
				String nifFlujo = "";
				// Se ha llegado hasta anexar -> Permitimos el pase			
			...
			if ( !StringUtils.isEmpty( scriptFirmante ) )
			{
				try
				{
					...
					if (StringUtils.isNotEmpty(firmante)){
						firmante = NifCif.normalizarDocumento(firmante);
						docInfo.setFirmante(firmante);
					}
				}
				catch( Exception exc )
				{
    ...
    private String evaluarEstadoPaso(int numpaso) throws Exception{
    	...
    	// Evaluamos paso (Completado / Pendiente / Pendiente Flujo )
    				...
    				if (doc.getObligatorio() != DocumentoFront.DEPENDIENTE &&
    					doc.getEstado() == DocumentoFront.ESTADO_CORRECTO &&
	    					doc.isFirmar() && !doc.isFirmado()){
    								// TODO Habría que ver si se permite flujo si remitimos a firma (como al q rellena)
	        						completado = PasoTramitacion.ESTADO_PENDIENTE;
	        						break;
            		}    				
    			}    			
    ...
    private HashMap evaluarParametrosPaso(int numpaso) throws Exception{
    	...
    	switch (paso.getTipoPaso()){
    		case PasoTramitacion.PASO_REGISTRAR:
    			...
    			// Generamos asiento (no podemos generar si hay notificacion y todavia no se ha seleccionado)
    			if ( tramiteInfo.getSeleccionNotificacionTelematica() != null || 
    				 tramiteInfo.getHabilitarNotificacionTelematica().equals(ConstantesSTR.NOTIFICACIONTELEMATICA_NOPERMITIDA)
    				){
	    				AsientoCompleto asiento = generaAsientoRegistral();
    ...
    private AsientoCompleto generaAsientoRegistral() throws Exception{    	        	    	
    	...
    	String xmlAsiento =  generarAsiento();    		    		
    ...
    private String generarAsiento() throws Exception{
    ...
    private String generarDatosPropios() throws Exception{
    	...
    	String strDatosPropios = GeneradorAsiento.generarDatosPropios(tramiteInfo,tramiteVersion,tramitePersistentePAD,plgForms,plgPagos); 
    ...
    //		- en caso de estar firmado comprueba quien debe firmarlo
    // 
    ...
    private ResultadoRegistrar procesoRegistrar(String asiento,FirmaIntf firma) throws Exception{
		...
		// - si se ha firmado por la persona que corresponde (añadimos firma al asiento en el RDS)
    	...
    	if (tramiteVersion.getDestino() == ConstantesSTR.DESTINO_CONSULTA) {    		
    		res = dest.consultar(tramiteVersion.getConsultaTipoAcceso(),tramiteVersion.getConsultaEJB(),tramiteVersion.getConsultaLocalizacion() == TramiteVersion.EJB_LOCAL ,tramiteVersion.getConsultaUrl(),
    				tramiteVersion.getConsultaAuth(),tramiteVersion.getConsultaAuthUser(),tramiteVersion.getConsultaAuthPwd(),this.datosFormularios);
    	}else {
	...
	private String evaluarScript(byte[] script,HashMap params) throws ProcessorException{		
		return ScriptUtil.evaluarScriptTramitacion(script,params,this.tramiteVersion,this.datosFormularios,this.tramitePersistentePAD,this.datosSesion);		
	}
	...
	private String evaluarScript(byte[] script,HashMap params, List modificados) throws ProcessorException{		
		return ScriptUtil.evaluarScriptTramitacion(script,params,this.tramiteVersion,this.datosFormularios,this.tramitePersistentePAD,this.datosSesion,false,modificados);		
	}
	...
	*/
	 ...
	 */
	private void obtenerDatosSesion(char nivAut,Locale idioma) throws Exception
	{				
	...
	private String actualizarPAD() throws Exception{
		...
		// Actualizamos PAD
		PadDelegate pad = DelegatePADUtil.getPadDelegate();		
		return pad.grabarTramitePersistente(this.tramitePersistentePAD);
	}
	...
	/**
	 * Verifica la firma del asiento registral. Debe estar firmado por el representante.
	...
	private boolean verificaFirmanteAdecuado( AsientoRegistral asientoRegistral,FirmaIntf firma ) throws ProcessorException
	{
		...
		return verificaFirmanteAdecuado( datosInteresado.getNumeroIdentificacion(), firma );
	}
	...
	private boolean verificaFirmanteAdecuado( String firmanteNecesario, FirmaIntf firma ) throws ProcessorException
	{
		if ( !StringUtils.isEmpty( firmanteNecesario ) )
		{
			...
			if ( !firmanteOk )
			{
				log.info( "Firmante incorrecto. Requerido [" + firmanteNecesario+ "]; Firmado con [" + nifFirma + "]" );
			}
			return firmanteOk;
		}
		return true;
