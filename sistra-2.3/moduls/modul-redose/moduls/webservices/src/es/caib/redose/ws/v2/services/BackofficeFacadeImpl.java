...
import es.caib.redose.ws.v2.model.BackofficeFacadeException;
import es.caib.redose.ws.v2.model.DocumentoRDS;
import es.caib.redose.ws.v2.model.FirmaWS;
import es.caib.redose.ws.v2.model.FirmasWS;
import es.caib.redose.ws.v2.model.ReferenciaRDS;
import es.caib.redose.ws.v2.model.TransformacionRDS;
...
@javax.jws.WebService(portName = "BackofficeFacade", serviceName = "BackofficeFacadeService", 
        targetNamespace = "urn:es:caib:redose:ws:v2:services", 
...
public class BackofficeFacadeImpl implements BackofficeFacade {
	...
	public DocumentoRDS consultarDocumentoFormateado(ReferenciaRDS referencia, String tipoPlantilla, String idioma) throws es.caib.redose.ws.v2.services.BackofficeFacadeException {
		try{
			es.caib.redose.modelInterfaz.ReferenciaRDS refInter = referenciaWSToReferenciaIntf(referencia);
			RdsDelegate rdsD = DelegateRDSUtil.getRdsDelegate();
	...
	public ReferenciaRDS insertarDocumento(DocumentoRDS documento) throws es.caib.redose.ws.v2.services.BackofficeFacadeException {
		try{
	...
	public ReferenciaRDS insertarDocumentoConTransformacion(DocumentoRDS documento, TransformacionRDS transformacion) throws es.caib.redose.ws.v2.services.BackofficeFacadeException {
		try{
			es.caib.redose.modelInterfaz.DocumentoRDS docInter = documentoWSToDocumentoInt(documento);
	...
	private DocumentoRDS documentoIntToDocumentoWS(es.caib.redose.modelInterfaz.DocumentoRDS doc) throws Exception{
		...
		if(doc != null){
			...
			if(doc.getIdioma() != null){
				docWS.setIdioma(new JAXBElement<String>(new QName("idioma"),String.class,doc.getIdioma()));
			}
			if(doc.getReferenciaGestorDocumental() != null){
				docWS.setReferenciaGestorDocumental(new JAXBElement<String>(new QName("referenciaGestorDocumental"),String.class,doc.getReferenciaGestorDocumental()));
			}
			if(doc.getCodigoDocumentoCustodia() != null){
				docWS.setCodigoDocumentoCustodia(new JAXBElement<String>(new QName("codigoDocumentoCustodia"),String.class,doc.getCodigoDocumentoCustodia()));
			}
	...
	private es.caib.redose.modelInterfaz.DocumentoRDS documentoWSToDocumentoInt(DocumentoRDS doc) throws Exception{
		...
		if(doc != null){
			...
			if(doc.getIdioma() != null){
				docInt.setIdioma(doc.getIdioma().getValue());
			}
			if(doc.getReferenciaGestorDocumental() != null){
				docInt.setReferenciaGestorDocumental(doc.getReferenciaGestorDocumental().getValue());
			}
			if(doc.getCodigoDocumentoCustodia() != null){
				docInt.setCodigoDocumentoCustodia(doc.getCodigoDocumentoCustodia().getValue());
			}
