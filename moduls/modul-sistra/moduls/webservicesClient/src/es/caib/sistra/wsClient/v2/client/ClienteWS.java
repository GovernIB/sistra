package es.caib.sistra.wsClient.v2.client;
...
public class ClienteWS {
	 ...
	 * @throws Exception
	 */
	public static ValoresDominio obtenerDominio(String url, String soapAction,String user, String pass, String idDominio,List parametros) throws Exception{
		
	   log.debug("Obteniendo dominio por webservice v2: url=" + url + " idDominio=" + idDominio);
		
	   // Generamos port para acceder al ws
	   es.caib.sistra.wsClient.v2.services.SistraFacade port = generarPort(url,soapAction,user,pass);

	   // Accedemos al ws y realizamos traduccion de parametros/resultado
       try {
        	es.caib.sistra.wsClient.v2.model.ParametrosDominio pd = new es.caib.sistra.wsClient.v2.model.ParametrosDominio();
           for(int i=0;i<parametros.size();i++){
        	   pd.getParametro().add((String)parametros.get(i));
           }
           es.caib.sistra.wsClient.v2.model.ValoresDominio vd = port.obtenerDominio(idDominio,pd);
           ValoresDominio vdInterfaz = valoresDominioWSToValoresDominioInt(vd);
           return vdInterfaz;
        } catch (es.caib.sistra.wsClient.v2.services.SistraFacadeException e) { 
	 ...
	 * @throws Exception
	 */
	public static List realizarConsulta(String url,String soapAction,String user,String pass,String identificadorTramite, Map datosFormulario) throws Exception {
		
		 log.debug("Realizando tramite consulta por webservice v2: url=" + url);
		 ...
		 try {
           ...
           es.caib.sistra.wsClient.v2.model.FormulariosConsulta forms = new es.caib.sistra.wsClient.v2.model.FormulariosConsulta();
           es.caib.sistra.wsClient.v2.model.FormularioConsulta form;
           for (Iterator it = datosFormulario.keySet().iterator();it.hasNext();){				
				String key = (String) it.next();
				form = new es.caib.sistra.wsClient.v2.model.FormularioConsulta();
           ...
           es.caib.sistra.wsClient.v2.model.DocumentosConsulta docs = port.realizarConsulta(identificadorTramite,forms);
        ...
        } catch (es.caib.sistra.wsClient.v2.services.SistraFacadeException exc) { 
	...
	private static es.caib.sistra.wsClient.v2.services.SistraFacade generarPort(String url,String soapAction,String user,String pass) throws Exception{
		...
		es.caib.sistra.wsClient.v2.services.SistraFacade port = service.getPort(PORT_NAME,es.caib.sistra.wsClient.v2.services.SistraFacade.class);
        
		// Configura puerto para autenticacion y paso por proxy
		WsClientSistraUtil.configurePort((BindingProvider)port,url,soapAction,user,pass);
	 ...
	 * @return
	 */
	private static ValoresDominio valoresDominioWSToValoresDominioInt(es.caib.sistra.wsClient.v2.model.ValoresDominio vd){
		Map columnas;
		es.caib.sistra.wsClient.v2.model.Fila fila;
