...
PLUGIN_LOG.debug(nom);<br />
	    ...
	    <li>Restricciones:
	      <ul>
	        ...
	        <li>Tambi&eacute;n se permite la expresi&oacute;n autorrellenable pero con la restricci&oacute;n de que s&oacute;lo puede contener referencias de campos de pantallas anteriores, no de la pantalla actual.
	            Para establecer los elementos de la lista se establece mediante la variable especial LISTAELEMENTOS que permite ir añadiendo elementos 
	...
	<ul><li><b>Referencia a una pantalla:</b><br/><br/>
	    ...
	    <ul>
          <li>El acceso a los dominios de SISTRA se hace a traves del plugin DominioSistraPlugin:<br />
              ...
              Los par&aacute;metros disponibles al instanciar este plugin son:
              	<ul>
              		...
              		<li>Par&aacute;metros dominio: array de parámetros (p.e. [f_nif])</li>
          ...
          res = new DominioSistraPlugin('TEST-PERS', [f_nif],'CODIGO','DESCRIPCION','CODIGOPADRE').execute(); -- Igual que la anterior pero permite establecer jerarqu&iacute;a entre los valores indicando que columna se utiliza para establecer el padre (esta columna contendrá el valor null para las filas "roots"). Esta columna para indicar el padre se utiliza para controles de selecci&oacute;n en &aacute;rbol. En caso de que este par&aacute;metro sea nulo se comportar&aacute; igual que la funci&oacute;n anterior<br/>
	...
	<b>Temas a tener en cuenta</b><br/>    
	   <ul>
	       ...
	       <li><strong>Importancia orden campos</strong><br/>En la ejecución de las expresiones se van recorriendo los campos en el orden definido en las pantallas, por tanto <strong>NO SE DEBE</strong> utilizar en una expresion de un campo la referencia de un campo posterior. Si no se cumple esta condición se pueden generar excepciones y errores ocultos de cálculo.<br/><br/></li>
	       <li><strong>CDT:Campo de Texto - Multilinea</strong><br/>Para devolver el valor del campo un multilinea hay que devolver un String con los saltos de línea (/r/n).<br/><br/></li>
	       <li><strong>Referencias al texto en campos indexados</strong><br/>Con f_campo_text se accede al text xa combo,radio,etc.<br/><br/></li>       
			<li><strong>Expresion autorellenable</strong>
				<ul>
					...
					<li>Para campos de tipo Lista de Elementos no puede contener referencias de campos de la misma pantalla y el valor se establece mediante una variable especial (ver descripción del control en Nuevas funcionalidades)</li>			
	            </ul>
		      <br/>
         </li>
       </ul>
...
<br/>
	<ul>
		<li><strong>CDT:Campo de Texto - Multilinea [NO SE PLANTEA SOLUCION HASTA NO SER NECESARIO]</strong><br/>FORMS genera XML con elemento con saltos de línea (mal generado ya que los saltos de línea en XML se obvian si no es CDATA) y luego no se cargan bien desde el XML inicial.</li>
	</ul>		      
</ul>   
...
<p><strong>Scripts</strong><br/>
  ...
  <br/>
  La expresión anterior nos indica que si se selecciona el componente iremos a "pdos", sino iremos a "ptres"<br/>
  <br/>
	
  <b><i>2. Dependencia entre Componentes. Valor de retorno: boleano / "readOnly" que indica la condición para que el componente esté activo o readonly.</i></b><br/>
  Permite establecer que un campo este activo o readonly en función de valores de otros campos:<br/>
  - Para indicar que este activo/inactivo devuelve un booleano (atención: si está inactivo el valor del campo no se toma en cuenta como dato).<br/>
  ...
  <i>
  <br/>
&nbsp;&nbsp;&nbsp;// En función del valor del campo c1 (enabled/disabled/readOnly) establece <br/>
...
&nbsp;&nbsp;&nbsp;res;</i>
  ...
  Deseamos que un campo determinado "suma" sea igual a la suma de los campos "valor1" y "valor2" de la pantella anterior "puno", con la siguiente expresión:<br/>
  <br/>
  <i>&nbsp;&nbsp;&nbsp;(f_puno_valor1 + f_puno_valor2)</i><br/>
  <br/>
el campo pasará a ser de solo lectura y su valor se calculará automaticamente con la expresión.</p>
	...
	<p><br/>
  <br/>
  ATENCIÓN: SI SE UTILIZAN DENTRO DE ESTA EXPRESIÓN REFERENCIAS A OTROS CAMPOS CALCULADOS DE LA MISMA PANTALLA HAY QUE TENER EN CUENTA EL ORDEN DEL CAMPO DENTRO DE LA PANTALLA YA QUE EN EL RESULTADO INFLUYE EL ORDEN DE EJECUCION DE LAS EXPRESIONES (VER 'TEMAS A TENER CUENTA') <br/>
  <br/>
	
  <b><i>4. Validación. Valor de retorno: boleano que indica si el campo es valido.</i></b><br/>
  Deseamos comprobar que la letra de un numero de identificación fiscal de un campo "nif" es correcta<br/>
  <br/>
  ...
  <b><i>5. Autorellenable. Valor de retorno: El mismo que el campo y su valor equivaldrá a la expresión.</i></b><br/>
  <br/>
