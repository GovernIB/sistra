...
import es.caib.bantel.front.util.NodoArbol;
import es.caib.bantel.front.util.UtilArbol;
...
public class FuncsNodesServiciosController implements Controller
{
    ...
    private List createNodos(HttpServletRequest request, List organosDestino)
    {
		...
		nodoRaiz.setDescripcion(getResources(request).getMessage((Locale) request.getSession().getAttribute(Globals.LOCALE_KEY),"notificacion.organo"));
    	result.add( nodoRaiz );
