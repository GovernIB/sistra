...
public class FuncsNodesUnidadesController implements Controller{
    ...
    public void perform(ComponentContext arg0, HttpServletRequest request, HttpServletResponse arg2, ServletContext arg3) throws ServletException, IOException
    {
        try {
            ...
            request.setAttribute( "unidadesadministrativas", createNodos(request,   Dominios.listarUnidadesAdministrativasArbol()));
        } catch (Exception e) {
	...
	private List createNodos(HttpServletRequest request, ValoresDominio unidadesAdministrativas)
    {
    	ArrayList result = new ArrayList();
		String parentId = "foldersTree"; 
    	NodoUnidadAdministrativa nodoRaiz = new NodoUnidadAdministrativa();
    	nodoRaiz.setId( parentId );
		nodoRaiz.setDescripcion(getResources(request).getMessage((Locale) request.getSession().getAttribute(Globals.LOCALE_KEY),"unidadesAdministrativas"));
    	result.add( nodoRaiz );
    	NodoUnidadAdministrativa nodo = null;

    	// Cogemos el primero que es el Padre de todos
    	
    	for(int i=0; i<unidadesAdministrativas.getFilas().size(); i++)
    	{
    		Map datos = (Map)unidadesAdministrativas.getFilas().get(i);
