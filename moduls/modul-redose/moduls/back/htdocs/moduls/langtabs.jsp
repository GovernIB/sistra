...
<table class="tabs" cellspacing="0" cellpadding="0" border="0">
    ...
    <logic:iterate id="lang" name="idiomas" type="java.lang.String">
       ...
       <logic:notEqual name="lang" value="<%=select%>">
          <td class="notab">
			<%if(request.getAttribute("estemAPlantilles") != null && "Y".equals(request.getAttribute("estemAPlantilles"))){ %>
			    <a onclick="if(validarFicherosGuardados('<bean:message key="errors.plantilla.modificar"/>')){reload('<%=lang%>');}" href="#"><%=new Locale(lang).getDisplayLanguage(current)%></a>
            <%}else{%>
