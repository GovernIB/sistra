...
create table STR_DOCUM  (
   ...
   DOC_PAD              NUMBER(20),
   DOC_MODELO           VARCHAR2(15)                    not null,
...
create table STR_DOMIN  (
   ...
   DOM_IDENTI           VARCHAR2(10)                    not null,
...
'Para tipo dominio Ejb indica si se debe realizar una autenticación explicíta o no (N) y como realizarla (de forma standar ( S ) o con el mecanismo de autenticación de la CAIB ( C ))';

comment on column STR_DOMIN.DOM_USR is
'Si el acceso es autenticado, indica el usuario.';

comment on column STR_DOMIN.DOM_PWD is
'Si el acceso es autenticado, indica la clave';
...
create table STR_TRAMIT  (
   ...
   TRA_IDENTI           VARCHAR2(10)                    not null
);
...
'Para trámite con destino Consulta indica si se debe realizar una autenticación explicíta o no (N) y como realizarla (de forma standar ( S ) o con el mecanismo de autenticación de la CAIB ( C ))';

comment on column STR_TRAVER.TRV_CONUSU is
'Para trámite con destino Consulta y con autenticación explicíta se indica usuario';

comment on column STR_TRAVER.TRV_CONPWD is
'Para trámite con destino Consulta y con autenticación explicíta se indica password';
