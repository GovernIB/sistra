...
var jst_image_folder = "./estilo_caib/images/treeview"
var jst_image_folder_user = "./estilo_caib/images/treeview/"
...
function renderChildren(strData, tblCls, menu){
	...
	for(var n in nodes){
		...
		var imag = '';		
		if (n0[1][2] != '' && n0[1][2] != undefined){
			imag = '<img' + (n0[1][4] ? ' class="action" onclick="showMenu(\'' + strData + '[' + n + ']\', this, event)"' : '') + ' src="' + jst_image_folder_user + n0[1][2] + '.png" alt="" />';
		}
		
		if(jst_context_menu && !n0[1][4] && !menu)
			n0[1][4] = jst_context_menu

		code.push(' alt="" /></td><td' + (n0[1][5] ? ' background="' + n0[1][5] + '"' : '') + '>' + check + imag + linkCheckIni + '<a ' + (n0[1][3] ? ' title="' + n0[1][3] + '"' : '') + ' onclick="nodeClick(this)" class="tree" href=' + "'" + (menu ? String(n0[1][0]).replace(/{@strData}/g, strData) : n0[1][0] ) + "'" + (n0[1][1] ? ' target="' + n0[1][1] + '"' : jst_target ? ' target="' + jst_target + '"' : '') + '>' + n0[0]  + '</a>' + linkCheckFin + '</td></tr>')
	}
	code.push('</table>')
